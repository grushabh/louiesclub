...
class MatchServices {
  ...
  getHeuristicsFilter(user) {
    ...
    const filters = esb.boolQuery().should([
      // breeds_list_text
      esb.boolQuery().must([
  ...
  getPersonasFilter(user) {
    ...
    if (user.hosting_page_complete__boolean === true && user.dog_added__boolean === true) {
      ...
      filters.push(
        esb.termQuery('hosting_page_complete__boolean', false)
      )
    } else {
      // match null?
    }
  ...
  async getQuery(user) {
    const query = esb.boolQuery().must(
      this.getPersonasFilter(user)
        ...
        .concat(
          esb.nestedQuery().path('dog_list_custom_dog').query(
            this.getHeuristicsFilter(user)
          )
        )
    )
